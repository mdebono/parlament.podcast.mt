...
class TestHttpPostRetry(unittest.TestCase):
    ...
    @patch('parlament.cache._session')
    def test_no_retry_on_success(self, mock_session, mock_sleep):
        mock_session.post.return_value = _response(200)
    ...
    @patch('parlament.cache._session')
    def test_retries_on_transient_403_then_succeeds(self, mock_session, mock_sleep):
        mock_session.post.side_effect = [_response(403), _response(200)]
        result = cache.httpPost('https://parlament.mt/test', None)
        self.assertEqual(result.status_code, 200)
    ...
    @patch('parlament.cache._session')
    def test_gives_up_after_persistent_403(self, mock_session, mock_sleep):
        mock_session.post.return_value = _response(403)
        result = cache.httpPost('https://parlament.mt/test', None)
        self.assertEqual(result.status_code, 403)
        self.assertEqual(mock_session.post.call_count, 1 + len(cache.RETRY_BACKOFF_SECONDS))

    @patch('parlament.cache.time.sleep')
    @patch('parlament.cache._session')
    def test_no_retry_on_404(self, mock_session, mock_sleep):
        mock_session.post.return_value = _response(404)
        result = cache.httpPost('https://parlament.mt/test', None)
        self.assertEqual(result.status_code, 404)
        self.assertEqual(mock_session.post.call_count, 1)
        mock_sleep.assert_not_called()
